...
`,g=[{team:1,name:"N/A"}],w=t=>t===void 0?"":g.map(a=>{if(a.team===t)return a.name}).join("");function v(t,a){const e={};return e[t]={id:t,played:0,mapPlayed:0,won:0,mapWon:0,lost:0,mapLost:0,points:0,scored:0,conceded:0},a.filter(s=>s.group===t).length===0||a.forEach(({team1:s,team2:l,games:o})=>{e[t].played++,o.forEach(({score1:i,score2:d})=>{let n=0;e[t].mapPlayed++,s===t?(e[t].scored+=i,e[t].conceded+=d,i>d?(e[t].mapWon++,n++):(e[t].mapLost++,n--)):l===t&&(e[t].scored+=d,e[t].conceded+=i,d>i?(e[t].mapWon++,n++):(e[t].mapLost++,n--)),n>0,e[t].won++,e[t].points+=3,e[t].lost++})}),e[t]}const y=[],L=m.map((t,a)=>{let e=t.kills/t.deaths||0,r="";a===0?r="bg-yellow-200":a===1?r="bg-gray-200":a===2&&(r="bg-amber-100");let s=g.find(o=>o.team===t.team);if(!s)return"";let l=v(s.team,y);return`
        <tr>
...
`,B=[{path:"/",data:h},{path:"/groups",data:f},{path:"/playoffs",data:u},{path:"/participants",data:x},{path:"/teams",data:k},{path:"/stats",data:$},{path:"/rules",data:S}],c=()=>{const t=document.getElementById("root");if(!t)return;const a=B.find(r=>r.path==window.location.pathname||"/fortnox-inhouse-tournament"+r.path==window.location.pathname);if(!a)return;const e=a.data;t.style.opacity="0.0",setTimeout(()=>{t.innerHTML=e,t.style.opacity="1.0"},50)},p=()=>{document.querySelectorAll("nav a").forEach(t=>{t.href===window.location.href?(t.classList.add("bg-emerald-950"),t.classList.remove("bg-transparent")):(t.classList.remove("bg-emerald-950"),t.classList.add("bg-transparent"))})},P=t=>{if(!t)return;t.preventDefault();const a=t.target;a&&(history.pushState({},"newUrl",a.href),c(),p())},E=()=>{window.addEventListener("popstate",c),window.addEventListener("DOMContentLoaded",()=>{document.querySelectorAll("nav a").forEach(t=>{t.addEventListener("click",P)}),c(),p()})};E();const C=`
