...
`,S=[{path:"/",data:y},{path:"/playoffs",data:A},{path:"/participants",data:E},{path:"/teams",data:R},{path:"/stats",data:O}],f=()=>{const t=document.getElementById("root");if(!t)return;const s=S.find(e=>e.path==window.location.pathname);if(!s)return;const r=s.data;t.style.opacity="0.0",setTimeout(()=>{t.innerHTML=r,t.style.opacity="1.0"},50)},b=()=>{document.querySelectorAll("nav a").forEach(t=>{t.href===window.location.href?(t.classList.add("bg-emerald-950"),t.classList.remove("bg-transparent")):(t.classList.remove("bg-emerald-950"),t.classList.add("bg-transparent"))})},I=t=>{if(!t)return;t.preventDefault();const s=t.target;s&&(history.pushState({},"newUrl",s.href),f(),b())},K=()=>{window.addEventListener("popstate",f),window.addEventListener("DOMContentLoaded",()=>{f(),b()}),document.querySelectorAll("nav ul li a").forEach(t=>{t.addEventListener("click",I)})};K();const C=`
