...
`,B=[{path:"/",data:p},{path:"/groups",data:f},{path:"/playoffs",data:u},{path:"/participants",data:x},{path:"/teams",data:k},{path:"/stats",data:$},{path:"/rules",data:S}],c=()=>{const t=document.getElementById("root");if(!t)return;const a=B.find(r=>r.path==window.location.pathname||"/fortnox-inhouse-tournament"+r.path==window.location.pathname);if(!a)return;const e=a.data;t.style.opacity="0.0",setTimeout(()=>{t.innerHTML=e,t.style.opacity="1.0"},50)},h=()=>{document.querySelectorAll("nav a").forEach(t=>{t.href===window.location.href?(t.classList.add("bg-emerald-950"),t.classList.remove("bg-transparent")):(t.classList.remove("bg-emerald-950"),t.classList.add("bg-transparent"))})},E=t=>{if(!t)return;t.preventDefault();const a=t.target;a&&(history.pushState({},"newUrl",a.href),c(),h())},P=()=>{window.addEventListener("popstate",c),window.addEventListener("DOMContentLoaded",()=>{document.querySelectorAll("nav a").forEach(t=>{t.addEventListener("click",E)}),c(),h()})};P();const R=`
